...
export default function ProblemSection() {
  ...
  return (
    <section id="problems" className="py-16 sm:py-24 bg-white overflow-hidden">
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <AlertCircle className="w-5 h-5 text-ink-muted/40" />
                  </div>

                  {/* Icon Badge */}
                  <div className={`w-14 h-14 rounded-2xl ${prob.bgClass} flex items-center justify-center mb-6`}>
                    <Icon className={`w-6 h-6 ${prob.iconColor}`} />
                  </div>

                  <h4 className="text-2xl font-bold text-ink mb-3">{prob.title}</h4>
                  <p className="text-ink-muted text-base leading-relaxed">{prob.desc}</p>
                </div>
              </motion.div>
            );
          })}
